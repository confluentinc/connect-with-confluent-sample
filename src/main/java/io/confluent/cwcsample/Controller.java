...
- (todo) LIST CLUSTERS: show clusters available within a onfluent Cloud environment
...
public class Controller {
    ...
    private ConcurrentHashMap<String,Connection> connectionsVault = new ConcurrentHashMap<>();

    //For this sample, we load some fake connections into the connectionsVault
    @PostConstruct
    private void init() {
        connectionsVault.put("S3BucketImport",new Connection("s3bucket","Dev - S3 Bucket", new HashMap<>()));
        connectionsVault.put("MongoDBImport",new Connection("mongodb","MongoDB Prod", new HashMap<>()));
    ...
    public ResponseEntity<Collection<Connection>> getConnections(){
        // Retrieve the list of connections from the connections vault
        //****REMINDER: Use a secure location for storing your connection information*****
        Collection<Connection> connections = connectionsVault.values();
    ...
    public ResponseEntity<String> addConnection(@PathVariable String connectionName, @RequestBody String body){

        //todo change request from string to JSON object to remove this conversion??
        ...
        Connection newConnection = new Connection("confluentcluster",connectionName,connectionDetails);

        // Add the connection to the connections vault
        //****REMINDER: Use a secure location for storing your connection information*****
        connectionsVault.put(connectionName, newConnection);

        // Return a success response
        return ResponseEntity.ok().body("");
    }

    //TODO: The below endpoints require a Confluent Cloud API Key/Secret (not kafka API key/secret)
    // so they require obtaining more credentials or using non API key/secrete based authentication
